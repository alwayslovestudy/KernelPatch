    ...
    if (args->local.data0 && args->ret > 0) {
        ...
        for (int i = 0; i < filter_rules.count; i++) {
            if (filter_rules.rules[i].fd == (int)args->local.data1) {
                long cplen = 0;
                char *buf = get_krl_func()->vmalloc(read_size + 1);
                if (buf) {
                    ...
                    if (cplen > 0) {
                        ...
                        if (pos) {
                            ...
                            //替换数据
                            if (str_replace_all(buf, filter_rules.rules[i].ori_data,
                                                filter_rules.rules[i].replace_data)) {
                                //将修改后的数据写回用户空间
                    ...
                    logke("file_filter vmalloc failed for size: %zu\n", read_size + 1);
                }
                break;
            }
        }
...
static void callback_before_openat(hook_fargs4_t *args, void *udata)
{
    ...
    args->local.data1 = (uint64_t)filename; //保存文件路径指针
}
